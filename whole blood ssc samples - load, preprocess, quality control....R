...
arrayQualityMetrics(expressionset = data_rma, outdir = "Report_for_ssc_whole_blood_data", force = TRUE)

# -------------------------------------------------------------------------------------------------------
# Preparing expression data for CIBERSORT/xCell analysis
# -------------------------------------------------------------------------------------------------------

# First, installing and loading Affymetrix Human Genome 219 Plate annotation data package, genefilter package

library(hgu219.db)
library(genefilter)

## CIBERSORT needs gene symbols as the row names (phenodata/sample data as the column names) ##

# Find the replicate genes and select the largest based on a normalized test statistic
unique_ids <- findLargest(rownames(data_exprs), rnorm(49386), "hgu219")

# Converting data matrix to a data frame
df_exprs <- as.data.frame(data_exprs)

# Extracting the data frame with only the statistically largest probe ids
final_df_exprs <- df_exprs[unique_ids, ]

# Extracting row names from data_exprs matrix
exprs_row_names <- rownames(final_df_exprs)

# Selecting and looking at GENE SYMBOLS, GENE NAMES and ENTREZ IDs from the probe ids
select(hgu219.db, keys = exprs_row_names, columns = c("ENTREZID", "SYMBOL", "GENENAME"), keytype = "PROBEID")

# Mapping probe ids to gene symbols
mapped_ids <- mapIds(hgu219.db, keys = exprs_row_names, column = c("SYMBOL"), keytype = "PROBEID")

# Creating a data frame with probe ids in column 1 and gene symbols in column 2
mapped_ids_df <- data.frame(keyName=names(mapped_ids), value=mapped_ids, row.names=NULL)

# Creating vector of gene symbols
gene_symbols <- mapped_ids_df[ ,2]

# Change row names of data_exprs matrix from probe ids to gene symbols
rownames(final_df_exprs) <- gene_symbols

# Change column names to sample annotation ("s" for sick, "h" for healthy)
colnames(final_df_exprs) <- new_pheno

# Now we're ready to output the matrix as a tab delimited file
write.table(final_df_exprs, file = "whole_blood_ssc_mixture_file.txt", sep = "\t", quote = FALSE)

### For the CIBERSORT analysis remember to add a column name for the gene symbol column (first column) ###
